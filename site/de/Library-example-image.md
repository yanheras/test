# Example Library Image - Your User Friendly Name for MyIG Here v0.2.0

## Library: Example Library Image (Experimentell) 

-------

**German**

-------

* * **Content: **image/png: ````Encoded data (6812 characters)````: **Id: **
  * ?: example-image
* * **Content: **image/png: ````Encoded data (6812 characters)````: **Version: **
  * ?: 0.2.0
* * **Content: **image/png: ````Encoded data (6812 characters)````: **Url: **
  * ?: [Example Library Image](Library-example-image.md)
* * **Content: **image/png: ````Encoded data (6812 characters)````: **Status: **
  * ?: draft
* * **Content: **image/png: ````Encoded data (6812 characters)````: **Experimental: **
  * ?: true
* * **Content: **image/png: ````Encoded data (6812 characters)````: **Date: **
  * ?: 2020-05-15 00:00:00+1000
* * **Content: **image/png: ````Encoded data (6812 characters)````: **Publisher: **
  * ?: HL7 International - [Some] Work Group
* * **Content: **image/png: ````Encoded data (6812 characters)````: **Jurisdiction: **
  * ?: 001



## Resource Content

```json
{
  "resourceType" : "Library",
  "id" : "example-image",
  "language" : "en",
  "url" : "http://somewhere.org/fhir/uv/myig/Library/example-image",
  "version" : "0.2.0",
  "name" : "ExampleLibraryImage",
  "title" : "Example Library Image",
  "status" : "draft",
  "experimental" : true,
  "type" : {
    "coding" : [{
      "system" : "http://terminology.hl7.org/CodeSystem/library-type",
      "code" : "asset-collection"
    }]
  },
  "date" : "2020-05-15T00:00:00.000+10:00",
  "publisher" : "HL7 International - [Some] Work Group",
  "contact" : [{
    "telecom" : [{
      "system" : "url",
      "value" : "http://hl7.org/Special/committees/[something]"
    }]
  }],
  "jurisdiction" : [{
    "coding" : [{
      "system" : "http://unstats.un.org/unsd/methods/m49/m49.htm",
      "code" : "001"
    }]
  }],
  "content" : [{
    "contentType" : "image/png",
    "data" : ""
  }]
}

```
